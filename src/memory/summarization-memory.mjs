...
async function summarizationMemoryDemo() {
  ...
  if (allMessages.length >= maxMessages) {
    ...
    // 清空历史消息，只保留最近的消息
    await history.clear();
    // 2. 将总结内容作为第一条 SystemMessage 存入历史记录
    await history.addMessage(
      new SystemMessage(`[之前对话的系统总结]：${summary}`),
    );
...
async function summarizeHistory(messages) {
  ...
  //getBufferString把一个包含消息对象（如 HumanMessage、AIMessage）的数组，
  //转换成一段便于人类阅读（以及便于输入给大模型）的纯文本对话记录
